...
export default async function Page({ params }) {
  ...
  return (
    <>
      ...
      <Section band="band-paper" title={P.commercialTitle}><div className="chain">{P.commercial.map((c) => <span key={c}>{c}</span>)}</div><Note>{P.boundary}</Note></Section>
      <Section band="band-carbon" title={P.disclosureTitle}>
