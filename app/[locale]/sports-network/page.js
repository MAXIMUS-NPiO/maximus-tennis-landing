...
export default async function Page({ params }) {
  ...
  return (
    <>
      ...
      <Section band="band-1" title={N.statusTitle}><span className="status agreement">{S.agreement}</span><p className="lead" style={{ marginTop: 14 }}>{N.statusP}</p><div className="btn-row"><Cta locale={locale} to="contact" label={N.cta} purpose="network" /></div></Section>
    </>
  );
}
