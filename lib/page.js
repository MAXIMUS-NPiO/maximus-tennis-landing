...
/** Dictionary subset for client form components (keeps the page payload small). */
export function formDict(dict, extra = []) {
  const out = {};
  for (const k of ["form", "common", "lead", "gps", ...extra]) out[k] = dict[k];
  return out;
}
