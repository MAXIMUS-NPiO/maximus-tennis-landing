    ...
    <>
      ...
      <Section first>
        <div className="grid-2">
          ...
          <div className="card"><p className="eyebrow">{Lg.entities.equipment.role}</p><h3>{entities.equipment.name}</h3><p>{Lg.entities.equipment.p}</p><div className="legal-block"><Row k={lb.licence} v={entities.equipment.licence} /><Row k={lb.vat} v={entities.equipment.vatTrn} /><Row k={lb.address} v={entities.equipment.address} /><div><a href={site.store} target="_blank" rel="noreferrer">{Lg.store}: maximussports.ae</a></div></div></div>
