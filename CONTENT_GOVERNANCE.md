...
`FOUNDER_CONFIRMED` · `MODELLED` · `TARGET` · `NOT_PROVIDED`. Public pages translate these into ordinary language ("Confirmed specification", "Calculated value", "Manufacturing target", "Not yet published"). A missing value is omitted or explained — never invented.
...
4. Product photography appears only when the exact asset version is APPROVED_FOR_PRODUCTION within its approved scope; until then sections are completed through typography and exact data.

## Review and release
1. Change in a branch or directly on `main` only after `npm test` and `npm run build` pass locally.
2. Preview: `npm start` + `npm run crawl -- http://localhost:3000`.
